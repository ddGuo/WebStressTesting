...
| `--proxy-check-target` / `--no-proxy-check-target` | 压测前用**目标地址**预检每个代理，只保留可达的（默认开） |
| `--proxy-check-timeout` | 目标预检单代理超时秒数（默认 3） |
| `--proxy-fail-threshold` | 压测中某代理失败达到该次数才本地剔除（默认 2，避免一次抖动误杀） |

- **一用户一 IP**：用户创建时分配一个代理并保持到结束；目标站点看到的出口 IP 稳定。
- **故障换绑**：某个代理连接失败时，该用户自动换用池内其他健康代理（不影响整体测试）；
  失败次数达到 `--proxy-fail-threshold` 后才本地剔除，避免瞬时抖动误杀。
- **目标预检**（默认开）：压测启动前用目标站点地址对每个代理做一次快速可达性探测
  （3s 超时、并发检测），**只保留对“目标站点”有效的代理**——这能大幅减少
  “对百度通则对目标不通”的免费代理带来的运行期失败；被剔除的代理计入
  `refined_out`，不污染 ok/fail 统计。
...
| `PROXYPOOL_SOURCES` | 全部 18 个（见第五节） | 免费源白名单（空=关闭爬取） |
